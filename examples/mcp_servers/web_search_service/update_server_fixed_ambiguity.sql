...
BEGIN
  ...
  IF v_server_id IS NULL THEN
    ...
    RAISE NOTICE 'Servidor creado con ID: %', v_server_id;
  ELSE
    ...
    RAISE NOTICE 'Servidor actualizado con ID: %', v_server_id;
  END IF;
  
  -- Verificar si la herramienta ya existe
  IF EXISTS (SELECT 1 FROM mcp_tools WHERE name = 'web_search' AND server_id = v_server_id) THEN
    ...
    SET 
      ...
      updated_at = NOW()
    WHERE name = 'web_search' AND server_id = v_server_id;
