...
BEGIN
  ...
  IF server_id IS NULL THEN
    ...
    RAISE NOTICE 'Servidor de clima creado con ID: %', server_id;
  ELSE
    ...
    RAISE NOTICE 'Servidor de clima actualizado con ID: %', server_id;
  END IF;
  
  -- Verificar si la herramienta ya existe
  IF EXISTS (SELECT 1 FROM mcp_tools WHERE name = 'get_weather_forecast' AND server_id = server_id) THEN
    ...
    SET 
      ...
      updated_at = NOW()
    WHERE name = 'get_weather_forecast' AND server_id = server_id;
